...
function rename_media_seo_activation()
{
    ...
    add_filter('attachment_fields_to_edit', 'add_image_seo_name_attachment_field_to_attachment_fields_to_edit', 10, 2);

    // Load the custom JS file
    add_action('admin_enqueue_scripts', 'enqueue_custom_admin_script');

    // Register the AJAX action to save the SEO name
    add_action('wp_ajax_save_seo_name', 'save_seo_name_ajax');

    // Hook to execute the update function when the event is scheduled
    add_action('update_content_image_references_event', 'update_content_image_references_background', 10, 4);
    ...
    add_action('clear_scheduled_update_content_image_references', 'clear_scheduled_update_content_image_references');
}
 ...
 */
function add_image_seo_name_attachment_field_to_attachment_fields_to_edit($form_fields, $post)
{
...
function save_seo_name_ajax()
{
    try {
        ...
        error_log("rinomina file: " . $seo_name);
        ...
        if (file_exists($new_file_path)) {
            throw new Exception(__('A file with the new name and the same file extension already exists. Change it or add a number!', 'fmrseo'));
        }
        ...
        if (!empty($metadata['sizes'])) {
            foreach ($metadata['sizes'] as $size => $size_data) {
                ...
                if (file_exists($old_thumbnail_path)) {
                    ...
                    schedule_update_content_image_references($old_thumbnail_url, $new_thumbnail_url, $seo_name, null);
                }
            }
        }
        ...
        schedule_update_content_image_references($old_file_url, $new_file_url, $seo_name, $post_id);

        wp_send_json_success([
            'message'  => __('File and thumbnails renamed successfully.', 'fmrseo'),
            'url'      => $new_file_url,
            'filename' => $seo_name . '.' . $file_ext
        ]);
    } catch (Exception $e) {
 ...
 */
function update_content_image_references_background($old_url, $new_url, $seo_name, $post_id)
{
    global $wpdb;
    error_log("POST");
    ...
    foreach ($posts as $post) {
        $updated_content = update_serialized_data($post->post_content, $old_url, $new_url);
    ...
    foreach ($postmeta as $meta) {
        $updated_meta_value = update_serialized_data($meta->meta_value, $old_url, $new_url);
 ...
 * @param string $seo_name The SEO name.
 */
function schedule_update_content_image_references($old_url, $new_url, $seo_name, $post_id)
{
 ...
 */
function clear_scheduled_update_content_image_references()
{
 ...
 */
function update_serialized_data($data, $old_value, $new_value)
{
    if (is_serialized($data)) {
        $unserialized_data = unserialize($data);

        $unserialized_data = update_value_in_array($unserialized_data, $old_value, $new_value);
 ...
 */
function update_value_in_array($array, $old_value, $new_value)
{
    foreach ($array as $key => $value) {
        if (is_array($value)) {
            $array[$key] = update_value_in_array($value, $old_value, $new_value);
